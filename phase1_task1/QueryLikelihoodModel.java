...
public class QueryLikelihoodModel {
  ...
  public static void main(String[] args) throws IOException {
    ...
    System.out.println("Enter query list file: ");
    s = br.readLine();
    ...
    PrintWriter writer = new PrintWriter("Lucene result.txt", "UTF-8");
    for (String queryString : queryList) {
      TopScoreDocCollector collector = TopScoreDocCollector.create(100, true);
      String[] split = queryString.split(" ", 2);
      try {
        Query q = new QueryParser(Version.LUCENE_47, "contents", analyzer).parse(split[1]);
        searcher.search(q, collector);
        ScoreDoc[] hits = collector.topDocs().scoreDocs;
        for (int i = 0; i < hits.length; ++i) {
          int docId = hits[i].doc;
          Document d = searcher.doc(docId);
          String[] docname = d.get("path").split("/");
          writer.println(split[0] + " Q0 " + docname[docname.length - 1].replace(".html", "") + " " + (i + 1) + " " + hits[i].score + " Lucene");
        }
      }
      catch (Exception e) {
